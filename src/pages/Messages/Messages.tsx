...
const Messages: React.FC<{ user: UserModel }> = ({ user }) => {
	...
	const messages = matches.map((match, index) => {
		const userId = match.userIds.find(matchedUser => matchedUser !== user.uid);
		...
		return <div className={classes.message} key={index} onClick={() => history.push(`/chat/${matchedUser?.uid}`)}>
			...
			<Typography variant="h5" data-testid="welcome-title">
				...
				<Typography className={classes.content} variant="body1" data-testid="welcome-title">
			{mostRecentMessage ? mostRecentMessage.messageContent : 'New match!'}
				</Typography>
			</div>
		</div>
	})
	const displayedMessages = messages.length ? messages : <Typography className={classes.title} variant="body1" data-testid="welcome-title">
		You have no matches yet, better get swiping!
	</Typography>;
	return (
		<div className={classes.messages}>
			<div className={classes.blockSpacing}>
				<Typography className={classes.title} variant="h3" data-testid="welcome-title">
					Messages
				</Typography>
				{displayedMessages}
			</div>
